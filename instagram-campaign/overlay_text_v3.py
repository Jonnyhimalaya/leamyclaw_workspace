#!/usr/bin/env python3
"""Add text overlay to headshot — V3 final with all fixes."""
...
# "LEAVING CERT" - light aqua accent
line1_font = ImageFont.truetype(font_bold, 150)
line1 = "LEAVING CERT"
bbox1 = draw.textbbox((0, 0), line1, font=line1_font)
w1 = bbox1[2] - bbox1[0]
x1 = W - w1 - 200
y1 = 380
draw.text((x1, y1), line1, fill='#8BBEC2', font=line1_font)
...
# "COURSE" - also white now (fixed contrast)
...
x4 = W - w4 - 200
y4 = y3 + 310
draw.text((x4, y4), line4, fill='white', font=course_font)

# Teal accent bar
bar_y = y4 + 180
bar_x = W - 200
draw.rectangle([x4, bar_y, bar_x, bar_y + 14], fill='#4A8C8C')

# --- Mid section: value props (right side, below header) ---
mid_font = ImageFont.truetype(font_bold, 110)
...
y_prop = bar_y + 80
for prop in props:
    bbox_p = draw.textbbox((0, 0), prop, font=mid_font)
    w_p = bbox_p[2] - bbox_p[0]
    x_p = W - w_p - 200
    draw.text((x_p, y_prop), prop, fill='#8BBEC2', font=mid_font)
    y_prop += 145
...
overlay = Image.new('RGBA', (W, banner_height), (15, 20, 30, 215))
img.paste(overlay, (0, banner_y), overlay)
draw = ImageDraw.Draw(img)

# Date - white
cta_font = ImageFont.truetype(font_bold, 135)
...
# Phone - white
phone_font = ImageFont.truetype(font_bold, 120)
...
# Brand - white, bigger
brand_font = ImageFont.truetype(font_bold, 110)
brand_text = "LEAMY MATHS · LIMERICK"
bbox_brand = draw.textbbox((0, 0), brand_text, font=brand_font)
w_brand = bbox_brand[2] - bbox_brand[0]
x_brand = (W - w_brand) // 2
y_brand = y_phone + 185
draw.text((x_brand, y_brand), brand_text, fill='white', font=brand_font)

# Save
output = '/home/jonny/.openclaw/workspace/instagram-campaign/images/post1-parents-v3.png'
img = img.convert('RGB')
