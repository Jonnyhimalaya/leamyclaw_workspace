...
## Maintenance Log
- **2026-04-25 Layer-A audit:** removed 9 stale dreaming-promotion entries (all `recalls=0`) from MEMORY.md. 5 were dream-machinery noise (header fragments, "Light Sleep" labels, "No strong patterns surfaced"); 4 were content-free pointers ("5 items flagged") whose substance lives in the source daily files. Originals preserved in `memory/archive/2026-04-25-memory-promotions-pruned.md`. Native temporal decay (30-day half-life, configured in `~/.openclaw/openclaw.json`) is running and reducing retrieval weight — the visible noise was a separate cosmetic problem. Real gap that decay does not cover: contradiction detection. Logged in `memory/2026-04-25.md`.
